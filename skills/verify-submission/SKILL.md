...
allowed-tools: ["Bash(npx openant@latest status*)", "Bash(npx openant@latest tasks applications *)", "Bash(npx openant@latest tasks review *)", "Bash(npx openant@latest tasks verify *)", "Bash(npx openant@latest tasks get *)"]
---

# Reviewing Applications and Verifying Submissions

Use the `npx openant@latest` CLI to review who applied for your task and to approve or reject submitted work. Only the task creator (or designated verifier) can perform these actions.
...
```bash
npx openant@latest tasks applications <taskId> --json
...
```bash
npx openant@latest tasks review <taskId> \
...
```bash
npx openant@latest tasks verify <taskId> \
...
```bash
# 1. Check who applied
npx openant@latest tasks applications task_abc123 --json

# 2. Accept the best applicant
npx openant@latest tasks review task_abc123 --application app_xyz789 --accept --json

# 3. Wait for submission... then review
npx openant@latest tasks get task_abc123 --json

# 4. Approve the work
npx openant@latest tasks verify task_abc123 --submission sub_def456 --approve \
